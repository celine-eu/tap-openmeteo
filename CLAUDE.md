# AGENTS.md - AI Agent Development Guide for tap-openmeteo
...
This tap follows the Singer specification and uses the Meltano Singer SDK to extract data from OpenMeteo.

### Key Components

1. **Tap Class** (`tap_openmeteo/tap.py`): Main entry point, defines streams and configuration
1. **Client** (`tap_openmeteo/client.py`): Handles API communication and authentication
1. **Streams** (`tap_openmeteo/streams.py`): Define data streams and their schemas
...
1. Define stream class in `tap_openmeteo/streams.py`
...
```python
class MyNewStream(OpenMeteoStream):
    name = "my_new_stream"
...
1. **SimpleHeaderPaginator**: For APIs using Link headers (RFC 5988)
   ...
   class MyStream(OpenMeteoStream):
...
```bash
tap-openmeteo --config config.json --discover
...
1. Update `config_jsonschema` in `tap_openmeteo/tap.py`
...
```bash
# .env.example
TAP_OPENMETEO_API_URL=https://api.example.com
TAP_OPENMETEO_API_KEY=your_api_key_here
TAP_OPENMETEO_BATCH_SIZE=100  # New setting
```
